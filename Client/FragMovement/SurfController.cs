...
namespace Fragsurf.Movement
{
    public class SurfController
    {
        ...
        public static bool nullNormal;
        public static bool allCastsShit;
        public static Vector3 lastGroundCheckNormal;
        ...
        public void SpawnDummyGroundObject()
        {
            dummyGO = new GameObject("SurfControllerDummyGroundObject");
            dummyGO.AddComponent<BoxCollider>();
        }
        ...
        private void CalculateFullMovement()
        {
            ...
            {
                CheckGrounded();
                //Debug.Log(Random.RandomRange(0,999999f)+"      "+ _surfer.MoveData.);
                //Debug.Log(groundObject==null);
                //GameObject.Find("DEBUG_TEST_TEXT").GetComponent<TextMeshProUGUI>().text =
                //       $"{(Cache.colliderCornerCaster.bottomCollision.normal.y)}  {(_surfer.MoveData.ForwardMove != 0 || _surfer.MoveData.SideMove != 0)}  {IsSurfing()}  {surfFixCd < 0f}  {_surfer.MoveData.Buttons.HasFlag(InputActions.Jump)}";
                if ((Cache.colliderCornerCaster.bottomCollision.normal.y >= SurfPhysics.SurfSlope) && /// Sliding bug fix
                    (_surfer.MoveData.ForwardMove != 0f || _surfer.MoveData.SideMove != 0f) &&
                    (IsSurfing())&&
                    !_surfer.MoveData.Buttons.HasFlag(InputActions.Jump))
                    //&&surfFixCd<0f)
                {
                    ForceGrounded();
                    // A constant check needs to now be done to determine what the angle of the current normal is.
                    // If player uses WSAD, the current normal is a SurfSlope and are currently surfing, they are forced into being grounded
                }
                else
                {
                    SurfPhysics.SurfSlope = 0.7f;
                }
            }
            surfFixCd -= Time.fixedUnscaledDeltaTime;

            CheckWater();
        }
        public static bool IsSurfing()
        {
            return groundObject == null;
        }
        public void ForceGrounded()
        {
            // This forces grounded behaviour
            _surfer.MoveData.Surfing = false;
            _surfer.MoveData.Sliding = false;
            _surfer.MoveData.SurfNormal = Vector3.zero;
            //Debug.Log(_surfer.MoveData.SurfNormal);
            SurfPhysics.SurfSlope = 0.1f;
            SetGround(dummyGO);
            surfFixCd = 0.1f;
        }
        ...
        private bool LadderMove()
        {
            ...
            else
            {
                ...
                //Debug.Log(_surfer.MoveData.Buttons.HasFlag(InputActions.MoveForward));
                //Debug.Log(rightSpeed + "    " + forwardSpeed);
        ...
        public bool CheckGrounded()
        {
            ...
            */
            ...
            lastGroundCheckNormal = trace.PlaneNormal;

            //Debug.Log(trace.PlaneNormal);

            if (trace.HitCollider != null)
            {
                //UnityEngine.Debug.Log(trace.PlaneNormal.y);
                nullNormal = false;
        ...
        {
            if (!_surfer.MoveData.Buttons.HasFlag(InputActions.Jump)
                || _surfer.GroundObject == null || _surfer.MoveData.JustJumped)
            {
                return;
            }
            ...
            SetGround(null);
            //Debug.Log(SurfCharacter.TICK_ID.ToString());
        }
        ...
        private Trace BoxCastToFloor(float distance = 0.05f, float extentModifier = 1.0f)
        {
            ...
            if (fines == 0) { allCastsShit = true; }
            //Debug.Log(bestHit.normal);
            return bestHit;
        }
